...
class oauth():
    ...
    def oauth_case1_request(self,newPath,myPrivateKey):
        sheet=4

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface5= testConfig.get_interface("INTERFACE5", "interface5")
        url05='{0}{1}'.format(url,interface5)

        logger.info("第5个接口的url是:{}".format(url05))
        ...
        logger.info("第5个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod_interface5(url05,simp_dict01)
