...
class authTokenQuery():
    ...
    def authTokenQuery_case1_request(self,newPath,myPrivateKey):
        sheet=3

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface4= testConfig.get_interface("INTERFACE4", "interface4")
        url04='{0}{1}'.format(url,interface4)

        logger.info("第4个接口的url是:{}".format(url04))
        ...
        logger.info("第4个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod(url04,simp_dict01)
