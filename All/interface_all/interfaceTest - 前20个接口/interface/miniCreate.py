...
class miniCreate():
    ...
    def miniCreate_case1_request(self,newPath,myPrivateKey):
        sheet=1

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface2= testConfig.get_interface("INTERFACE2", "interface2")
        url02='{0}{1}'.format(url,interface2)

        logger.info("第2个接口的url是:{}".format(url02))
        ...
        logger.info("第2个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod(url02,simp_dict01)
