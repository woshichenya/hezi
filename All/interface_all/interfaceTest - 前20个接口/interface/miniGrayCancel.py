...
#######################################################
class miniGrayCancel():


    def miniGrayCancel_case1_request(self,newPath,myPrivateKey):
        sheet=15

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface16= testConfig.get_interface("INTERFACE16", "interface16")
        url16='{0}{1}'.format(url,interface16)

        logger.info("第16个接口的url是:{}".format(url16))
        ...
        logger.info("第16个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod(url16,simp_dict01)
