...
class userAuth():
    ...
    def userAuth_case1_request(self,newPath,myPrivateKey):
        sheet=5

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface6= testConfig.get_interface("INTERFACE6", "interface6")
        url06='{0}{1}'.format(url,interface6)

        logger.info("第6个接口的url是:{}".format(url06))
        ...
        logger.info("第6个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.getMethod(url06,simp_dict01)
