...
class queryMembers():
    ...
    def queryMembers_case1_request(self,newPath,myPrivateKey):
        sheet=8

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface9= testConfig.get_interface("INTERFACE9", "interface9")
        url09='{0}{1}'.format(url,interface9)

        logger.info("第9个接口的url是:{}".format(url09))
        ...
        logger.info("第9个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod(url09,simp_dict01)
