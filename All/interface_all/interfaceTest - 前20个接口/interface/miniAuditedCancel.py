...
#######################################################
class miniAuditedCancel():


    def miniAuditedCancel_case1_request(self,newPath,myPrivateKey):
        sheet=19

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface20= testConfig.get_interface("INTERFACE20", "interface20")
        url20='{0}{1}'.format(url,interface20)

        logger.info("第20个接口的url是:{}".format(url20))
        ...
        logger.info("第20个接口的入参是:{}".format(simp_dict01))

        myGet=common.requestMethod.requestMethod()
        myRequest_dict=myGet.getMethod(url20,simp_dict01)
