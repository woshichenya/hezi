...
#######################################################
class miniVersionDetailQuery():


    def miniVersionDetailQuery_case1_request(self,newPath,myPrivateKey):
        sheet=18

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface19= testConfig.get_interface("INTERFACE19", "interface19")
        url19='{0}{1}'.format(url,interface19)

        logger.info("第19个接口的url是:{}".format(url19))
        ...
        logger.info("第19个接口的入参是:{}".format(simp_dict01))

        myGet=common.requestMethod.requestMethod()
        myRequest_dict=myGet.getMethod(url19,simp_dict01)
