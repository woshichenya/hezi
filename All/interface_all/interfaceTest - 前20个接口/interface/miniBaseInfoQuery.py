...
#######################################################
class miniBaseInfoQuery():


    def miniBaseInfoQuery_case1_request(self,newPath,myPrivateKey):
        sheet=13

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface14= testConfig.get_interface("INTERFACE14", "interface14")
        url14='{0}{1}'.format(url,interface14)

        logger.info("第14个接口的url是:{}".format(url14))
        ...
        logger.info("第14个接口的入参是:{}".format(simp_dict01))

        myGet=common.requestMethod.requestMethod()
        myRequest_dict=myGet.getMethod(url14,simp_dict01)
