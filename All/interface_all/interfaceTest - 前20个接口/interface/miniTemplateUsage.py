...
#######################################################
class miniTemplateUsage():


    def miniTemplateUsage_case1_request(self,newPath,myPrivateKey):
        sheet=17

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface18= testConfig.get_interface("INTERFACE18", "interface18")
        url18='{0}{1}'.format(url,interface18)

        logger.info("第18个接口的url是:{}".format(url18))
        ...
        logger.info("第18个接口的入参是:{}".format(simp_dict01))

        myGet=common.requestMethod.requestMethod()
        myRequest_dict=myGet.getMethod(url18,simp_dict01)
