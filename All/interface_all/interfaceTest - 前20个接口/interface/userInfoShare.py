...
class userInfoShare():
    ...
    def userInfoShare_case1_request(self,newPath,myPrivateKey):
        sheet=6

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface7= testConfig.get_interface("INTERFACE7", "interface7")
        url07='{0}{1}'.format(url,interface7)

        logger.info("第7个接口的url是:{}".format(url07))
        ...
        logger.info("第7个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod(url07,simp_dict01)
