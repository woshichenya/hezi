...
#######################################################
class miniBaseInfoModify():


    def miniBaseInfoModify_case1_request(self,newPath,myPrivateKey):
        sheet=14

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface15= testConfig.get_interface("INTERFACE15", "interface15")
        url15='{0}{1}'.format(url,interface15)

        logger.info("第15个接口的url是:{}".format(url15))
        ...
        logger.info("第15个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod(url15,simp_dict01)
