...
#######################################################
class miniVersionListQuery():


    def miniVersionListQuery_case1_request(self,newPath,myPrivateKey):
        sheet=16

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface17= testConfig.get_interface("INTERFACE17", "interface17")
        url17='{0}{1}'.format(url,interface17)

        logger.info("第17个接口的url是:{}".format(url17))
        ...
        logger.info("第17个接口的入参是:{}".format(simp_dict01))

        myGet=common.requestMethod.requestMethod()
        myRequest_dict=myGet.getMethod(url17,simp_dict01)
