...
class agentCreate():
    ...
    def agentCreate_case1_request(self,newPath,myPrivateKey):
        sheet=0

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface1= testConfig.get_interface("INTERFACE1", "interface1")
        url01='{0}{1}'.format(url,interface1)

        logger.info("第1个接口的url是:{}".format(url01))
        ...
        logger.info("第1个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod_interface1(url01,simp_dict01)
...
aaa=mytest.agentCreate_case1_request(path_12,key_12)
print(aaa)
'''
