...
#######################################################
class miniExperienceCreate():


    def miniExperienceCreate_case1_request(self,newPath,myPrivateKey):
        sheet=11

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface12= testConfig.get_interface("INTERFACE12", "interface12")
        url12='{0}{1}'.format(url,interface12)

        logger.info("第12个接口的url是:{}".format(url12))
        ...
        logger.info("第12个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.getMethod(url12,simp_dict01)
