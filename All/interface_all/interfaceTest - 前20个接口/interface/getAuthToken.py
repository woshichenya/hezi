...
class getAuthToken():
    ...
    def getAuthToken_case1_request(self,newPath,myPrivateKey):
        sheet=2

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface3=testConfig.get_interface("INTERFACE3", "interface3")
        url03='{0}{1}'.format(url,interface3)

        logger.info("第3个接口的url是:{}".format(url03))


        ################################
        myExcelNew = common.excelHandle.excelHandle(newPath)
        myMd5 = common.md5Handle.md5Handle()
        dict = myExcelNew.excelToDir(newPath,sheet,1)
        ...
        logger.info("第3个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod_interface3(url03,simp_dict01)
...
'''
mytest=getAuthToken()
path_12='G:\\autoTestInterface\\interfaceTest\\testCaseExcel\\apipayOpen_1545367361615.xls'
...
aaa=mytest.getAuthToken_case1_request(path_12,key_12)
print(aaa)
'''
