...
#######################################################
class miniExperienceQuery():


    def miniExperienceQuery_case1_request(self,newPath,myPrivateKey):
        sheet=12

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface13= testConfig.get_interface("INTERFACE13", "interface13")
        url13='{0}{1}'.format(url,interface13)

        logger.info("第13个接口的url是:{}".format(url13))
        ...
        logger.info("第13个接口的入参是:{}".format(simp_dict01))

        myGet=common.requestMethod.requestMethod()
        myRequest_dict=myGet.getMethod(url13,simp_dict01)
