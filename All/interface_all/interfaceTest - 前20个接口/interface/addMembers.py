...
class addMembers():
    ...
    def addMembers_case1_request(self,newPath,myPrivateKey):
        sheet=7

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface8= testConfig.get_interface("INTERFACE8", "interface8")
        url08='{0}{1}'.format(url,interface8)

        logger.info("第8个接口的url是:{}".format(url08))
        ...
        logger.info("第8个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod(url08,simp_dict01)
