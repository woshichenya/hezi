...
class deleteMembers():
    ...
    def deleteMembers_case1_request(self,newPath,myPrivateKey):
        sheet=9

        testConfig = common.configHandle.getConfig()
        url = testConfig.get_url()

        interface10= testConfig.get_interface("INTERFACE10", "interface10")
        url10='{0}{1}'.format(url,interface10)

        logger.info("第10个接口的url是:{}".format(url10))
        ...
        logger.info("第10个接口的入参是:{}".format(simp_dict01))

        myPost=common.requestMethod.requestMethod()
        myRequest_dict=myPost.postMethod(url10,simp_dict01)
